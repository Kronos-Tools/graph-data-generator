...
const Util = {
	...
	writeObjectFile(destinationPath, object, objectName, objectType) {
		...
		/**
		 * Returns an array of random element from an array of elements. The element taken will be removed from the array
		 * @param elementArray (Array) This array contains all the possible elements. A taken element will be removed from the array
		 * @param amaount (number) The amount of elements to be created
		 * @return elements (Array) The elements taken
		 */
		getRandomElements(elementArray, amount) {
			if (amount > elementArray.length) {
				throw "Should take more elements as available";
			}
			const res = [];
			while (res.length < amount) {
				res.push(this.getRandomElement(elementArray));
			}
