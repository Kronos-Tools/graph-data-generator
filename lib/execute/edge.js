...
module.exports = function (model, nextId, logger) {
	...
	Object.keys(model.edges).forEach(edgeName => {
		...
		const edgeObject = _createEdge(model, logger, edgeConfig, model.edge_custom_functions);
...
function _createEdge(model, logger, edgeConfig, customFunctions) {
