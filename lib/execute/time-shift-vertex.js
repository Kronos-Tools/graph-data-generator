...
function moveEdgesToIteration(model, logger, newEdge, currentIteration) {
	...
	if (newEdge.objects) {
		if (currentIteration === 'start') {
			...
			existingEdge.time_shift_store.start = newEdge.objects;
...
function startIteration(model, logger, verticesConfig, parentVertex) {
	...
	Object.keys(verticesConfig).forEach((vertexName) => {
		...
		if (start) {
			if (parentVertex) {
				const addedElements = shiftParentChildEdgeAdd(model, logger, parentVertex, vertexConfig, 'start');
