	...
	if (elementsPerSource || (availableTargets && availableTargets.length > 0)) {
		sourceIds.forEach(srcId => {
			...
			if (availableTargets && availableTargets.length > 0) {
				...
				const err = {
					"message": `No targets available for edge '${edgeConfig.name}' and source id '${srcId}'`
						//"object": edgeConfig
				};
				logger.debug(err);
			}
		...
		const err = {
			"message": `No targets available for edge '${edgeConfig.name}'`
		};
		logger.warn(err);
	}
...
function createValues(logger, edgeConfig, count, availableTargets, usedTargets) {
	...
	if (usedTargets) {
		...
		if (availableTargets.length < count) {
			const err = {
				"message": `Not enaught available targets. Requested are '${count}', but available are only '${availableTargets.length}' for edge '${edgeConfig.name}'.`
			};
			logger.warn(err);
...
function calculateEdgeTargetCounts(model, logger, edgeConfig, customFunctions) {
	...
	if (!sourceIds || sourceIds.length === 0) {
		const err = {
			"message": `Could not get any source ids for edge '${edgeConfig.name}'`
		};
		logger.warn(err);
		return;
	}
	...
	if (count > sourceIds.length * max) {
		const err = {
			"message": `'count_all'=${count} exceeds the amount of source*max=${sourceIds.length*max} (source='${sourceIds.length}', max='${max}') in edge '${edgeConfig.name}'`,
			"object": edgeConfig
		};
		logger.error(err);
		throw new Error(err);
	...
	while (elementCounter < count) {
		...
		if (lastCount === elementCounter) {
			...
			const err = {
				"message": `Could not increase the value over ${elementCounter} but expetced are ${count} for edge '${edgeConfig.name}'.`
			};
			logger.warn(err);
			break;
		} else {
