/* jslint node: true, esnext: true */
"use strict";

const fs = require('fs');
const path = require("path");
const csvWriter = require('csv-write-stream');


class ExporterCsv {
	constructor(exporter, config) {
		if (!config.target_dir) {
			throw new Error("No 'target_dir' defined");
		}
		if (!config.file_name) {
			const err = {
				"message": `No 'file_name' defined in export config.`,
				"object": config
			};
			exporter.error(err);
			throw new Error(err);
		}

		if (!config.fields) {
			const err = {
				"message": `No 'fields' defined in export config.`,
				"object": config
			};
			exporter.error(err);
			throw new Error(err);
		}

		this.target_dir = config.target_dir;

		// An hash with the provided exporter functions
		this.config = config;

		this.init();
	}

	/**
	 * Initializes the writer
	 */
	init() {
		const writerConfig = {};

		if (this.config.header) {
			writerConfig.header = this.config.header;
		}
		if (this.config.seperator) {
			writerConfig.seperator = this.config.seperator;
		}
		if (this.config.newline) {
			writerConfig.newline = this.config.newline;
		}

		const writer = csvWriter(writerConfig);

		const ws = fs.createWriteStream(path.join(this.target_dir, this.config.file_name));
		writer.pipe(ws);

		this.writer = writer;
	}


	/**
	 * writes the chunk data or parts of it
	 * @param chunk (object) The chunk data
	 */
	write(chunk) {
		// Write the row
		const r = [];
		const fields = this.config.fields;

		fields.forEach(fieldName => {
			r.push(chunk[fieldName]);
		});

		this.writer.write(r);
	}

	/**
	 * Closes the writer
	 */
	close() {
		this.writer.end();
	}
}

/**
 * Initializes the custom writer
 * @param exporter (object) The exporter object
 * @param config (object) The exporter configuration
 * @return writer (object) New created custom writer
 */
module.exports.init = function (exporter, options) {
	return new ExporterCsv(exporter, options);
};

module.exports.ExporterCsv = ExporterCsv;
