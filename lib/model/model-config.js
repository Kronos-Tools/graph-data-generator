...
const validateTimeShift = require('./model-validate-time-shift');
...
class ModelConfig extends Logger {
	...
	validate() {
		...
		if (!validateTimeShift(this, this)) {
			isOk = false;
		}
		return isOk;
	}
	...
	buildVertexRegistry(vertices) {
		Object.keys(vertices).forEach(vertexName => {
			...
			if (vertex.vertices) {
				...
				this.buildVertexRegistry(vertex.vertices);
			}
		});
	}
	...
	getVertex(name, failOnError) {
		return this._getConfigElement(this.vertices, 'Vertex', name, failOnError);
	}
