...
class TdgExecuter extends llm.LogLevelMixin(_TdgExecuter, llm.defaultLogLevels, llm.defaultLogLevels.info) {
	...
	constructor(opts) {
		...
		// store all the custom functions.
		if (opts.functions) {
	...
	createVertices() {
		...
		this.model.vertices.forEach(vertex => {
			...
			promises.push(
				this.writer.writeVertex(vertex).then(() => {
	...
	createEdges() {
		...
		this.model.edges.forEach(edge => {
			...
			promises.push(
				this.writer.writeEdge(edgeObject).then(() => {
					this.afterEdge(edgeObject);
					return Promise.resolve();
				})
			);
		});
	...
	createEdge(edgeConfig) {
		...
		if (edgeConfig.tdg.src && edgeConfig.tdg.src.function) {
			sourceIds = this.functions[edgeConfig.tdg.src.function](this, edgeConfig);
		} else {
		...
		if (!elementsPerSource) {
			if (edgeConfig.tdg.target && edgeConfig.tdg.target.function) {
				...
				if (!this.functions[edgeConfig.tdg.target.function.elements]) {
					const err = {
				...
				availableTargets = this.functions[edgeConfig.tdg.target.function.elements](this, edgeConfig);
			} else {
		...
		sourceIds.forEach(srcId => {
			...
			if (elementsPerSource) {
				if (edgeConfig.tdg.target && edgeConfig.tdg.target.function) {
					// there is a custom function defined.
					availableTargets = this.functions[edgeConfig.tdg.target.function.elements](this, edgeConfig, srcId);
				} else {
					availableTargets = this.targetIds(edgeConfig);
				}
				usedTarget = new Set();
	...
	calculateEdgeTargetCounts(edgeConfig) {
		...
		if (edgeConfig.tdg.src && edgeConfig.tdg.src.function) {
			// there is a custom function defined.
			if (!this.functions[edgeConfig.tdg.src.function]) {
				const err = {
					"message": `The custom function '${edgeConfig.tdg.src.function}' to get the source IDs is not defined`,
					"object": edgeConfig
				};
				this.error(err);
				throw new Error(err);
		...
		if (!elementsPerSource) {
			if (edgeConfig.tdg.target && edgeConfig.tdg.target.function) {
				...
				if (!this.functions[edgeConfig.tdg.target.function.amount]) {
					const err = {
				...
				targetIdsLength = this.functions[edgeConfig.tdg.target.function.amount](this, edgeConfig);
			} else {
		...
		while (elementCounter < count) {
			...
			sourceIds.forEach(srcId => {
				...
				if (elementsPerSource) {
					if (edgeConfig.tdg.target && edgeConfig.tdg.target.function) {
						// there is a custom function defined.
						targetIdsLength = this.functions[edgeConfig.tdg.target.function.amount](this, edgeConfig, srcId);
					} else {
						targetIdsLength = this.targetIdsLength(edgeConfig);
					}
				}
