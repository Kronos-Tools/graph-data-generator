...
class ExporterDefault {
	...
	_run() {
		...
		for (let i = 0; i < interationCount; i++) {
			if (i === 0) {
				// Start
				this._createSourceData('start');
				this._createColumnData('start');
			}
	...
	_exportCurrentIteration(currentIteration) {
		...
		exporterWriterConfigs.forEach((writerConfig) => {
			...
			writerConfigNew.file_name = self._getTargetFileName(currentIteration, self.config);
	...
	_getTargetFileName(currentIteration, config) {
		const sourceName = config.data.sources[0];
	...
	_createSourceData(currentIteration) {
		...
		sources.forEach((sourceName) => {
			...
			if (currentIteration === 'start') {
				ids = vertex.time_shift_store.start;
			} else {
				ids = vertex.time_shift_store.iter[currentIteration].a;
			}
			if (ids) {
				ids.forEach((id) => {
					if (!self.store[id]) {
	 ...
	 */
	_createColumnData(currentIteration) {
		const config = this.config;
	...
	_removeColumnData(currentIteration) {
		...
		let ids;
		if (currentIteration !== 'start') {
			ids = vertex.time_shift_store.iter[currentIteration].r;
			if (ids) {
				ids.forEach((id) => {
